...
/**
 * The reason when git itself could not answer what the checkout changed.
 *
 * DECLARED HERE with the other unproven reasons, and not at the call site: the Action reads the two
 * file lists and this module owns every sentence about what they mean, so a reader who follows one
 * of these into a file finds all of them together (messages plan S3).
 */
export function checkoutNotComparableReason(checkoutSha) {
    return `abloh could not compare the checkout ${short(checkoutSha)} with this pull request's head`;
}
/**
