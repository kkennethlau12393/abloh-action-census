...
export function planLimitSentence(facts) {
    /* THE FULL STOP IS IN THE SENTENCE AND NOT AT THE REGISTRY'S DOOR (round 2, rule 9). `refusal.ts`
       composes every summary through `composedSummary`, which puts one on where it is missing - but
       this module is rendered into the ACTION by `pnpm gen:action`, and the Action composes the
       sentence directly with no such door. A stop added only at the door would make the two copies
       differ by a character, which `wire-protocol-e2e.test.ts` reads over a real wire. */
