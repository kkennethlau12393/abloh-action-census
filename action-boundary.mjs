...
import { ActionRefusalError } from "./action-refusal.mjs";
import { ACTION_PIN_REMEDY, PR_COMMENT_REMEDY, prCommentUnavailableNotice } from "./action-notices.generated.mjs";
import { narrationTo } from "./narration.generated.mjs";
import { refusalCode, refusalMessage } from "./refusal-envelope.mjs";

/** This Action's account of what it did. The channel's rule is in `packages/core/src/narration.ts`. */
const narrate = narrationTo((line) => {
...
/*
 * WHERE THE CACHED DIRECTORIES LIVE, from the file that also writes both halves of every cache step.
 *
 * The directory is one of the four facts a restore step and a save step have to agree about, and it
 * is the one this boundary used to decide for itself - under the run id, so every entry was saved at
 * a path no later run would ask for. `packages/core/src/action-caches.ts` owns it now and this is
 * the copy the runner runs.
 */
import {
  ACTION_CACHES,
  ACTION_CACHE_STATE_ROOT,
  actionCacheSegments,
} from "./action-caches.generated.mjs";
/*
 ...
 */
import {
  MODEL_SERVICE_BASE_PATH,
  MODEL_SERVICE_ROUTE_PATHS,
  RUN_REFUSAL_LIMITS,
  SETUP_TRIAL_MAX_BYTES,
...
/**
 * WHERE STATE THAT MUST NOT BE SHARED BETWEEN RUNS GOES, under `RUNNER_TEMP` and under this run.
 *
 * A root of its own rather than a directory beside the cached ones, because the two rules are
 * opposite: {@link ACTION_CACHE_STATE_ROOT} holds directories whose path must repeat so a later run
 * can restore what an earlier one saved, and this holds the coverage provider directory, which holds
 * executable code this run will execute and is never restored from anywhere.
 */
const RUN_STATE_ROOT = "abloh-run-state";
...
function fail(message) {
  throw new ActionRefusalError(`abloh Action boundary: ${message}`);
}
...
export function validateActionInputs(environment = process.env) {
  ...
  if (optional(environment.PR_COMMENT, "pr-comment") !== "") {
    fail(`${prCommentUnavailableNotice()}. ${PR_COMMENT_REMEDY}`);
  }
...
function reachBaseCommit(repositoryRoot, base, environment) {
  ...
  if (!isShallow()) {
    ...
    narrate(`abloh: the base commit ${baseTarget} is not in this checkout. Fetching it from ${remote}`);
...
function ensureCommitPresent(repositoryRoot, sha, environment) {
  ...
  narrate(`abloh: the pull request's head commit ${sha} is not in this checkout. Fetching it from ${remote}`);
  gitQuiet(
    repositoryRoot,
...
function proveCheckoutIdentity(repositoryRoot, checkoutSha, headSha, baseSha, environment) {
  ...
  if (changedPaths === null || divergedPaths === null) {
    return {
      kind: "unproven",
      conflicts: [],
      reason: checkoutNotComparableReason(checkoutSha),
    };
  }
...
/**
 * WHAT THE LEAF OF A PRIVATE STAGING PATH MAY MEET, and every caller states which.
 *
 * `reuse` is the ancestors' rule: a directory that is already there is walked through.
 *
 * `new` is for a path named for THIS run, where anything already sitting at it is a collision
 * nobody can explain, so the run refuses rather than measuring in it.
 *
 * `fresh` is for a path every run spells the same way, which is what the three cached directories
 * have to be: `actions/cache` versions an entry by the paths it covers, so a directory named for the
 * run writes an entry no later run can ask for. Such a leaf can meet what an earlier job left
 * behind, and `new`'s refusal would be a refusal of the repeat itself - so it is removed and remade,
 * and the only records the run measures with are the ones its own restore step put there.
 * `RUNNER_TEMP` is emptied at the start of each job, so on a compliant runner there is nothing to
 * remove; where a runner keeps it, the guarantee still holds.
 */
function privateDirectory(parent, parts, leafPolicy = "reuse") {
  ...
  for (const [index, part] of parts.entries()) {
    ...
    const policy = index === parts.length - 1 ? leafPolicy : "reuse";
    const next = join(current, part);
    /* The removal is the whole of what `fresh` means, and it is safe to write because `next` is a
       validated segment under an ancestor this loop has already canonicalized inside RUNNER_TEMP.
       `rmSync` does not follow the entry it is given, so a symlink left here is unlinked rather than
       followed - and the check two lines below refuses one that arrives any other way. */
    if (policy === "fresh") rmSync(next, { recursive: true, force: true });
    const existed = existsSync(next);
    if (policy === "new" && existed) fail(`private staging leaf already exists: ${next}`);
...
export function assertSupportedNodeVersion(version = process.versions.node) {
  ...
  if (!supportedNodeVersion(version)) {
    fail(`Node ${version} is unsupported, and abloh could not provision a Node ${nodeFloor().text} runtime of its own`);
  }
}
...
export function preflight(environment = process.env) {
  ...
  if (identity.kind === "unproven") {
    ...
    narrate(
      "abloh: this run reports nothing. The refusal above stands and no evidence is produced " +
        "or uploaded. A diagnostic sweep was asked for, so abloh runs its stages over the tree that " +
        "is here and writes a record saying which of them would have worked.",
    );
  ...
  for (const candidate of [
    ...
    join(runnerTemp, ACTION_CACHE_STATE_ROOT),
    join(runnerTemp, RUN_STATE_ROOT, `${runId}-${runAttempt}`),
  ...
  /*
   * THE STATE THAT SURVIVES A RUN: exact-commit baseline history, the marigold store - the
   * carry-forward records, the predictor's described outcomes, pool 2's pinned bugs, a carried bug
   * and the line map - and the triage verdicts a customer already paid for.
   *
   * RESTORED FROM THE GITHUB CACHE, unlike the coverage provider directory below - and the
   ...
   * that trusted. These hold DATA whose only use is bounded by rules that hold whatever the data
   * says: a carried triage verdict can only ever keep a gap open, because a verdict that would
   * remove one is re-asked live every run; a carried candidate is proposed source that is executed,
   * proved and suite-checked fresh here; a carried bug is re-applied and re-run before anything is
   * claimed about it; a stored prediction only chooses what to spend execution on, never what to
   * report. Every record is re-validated on read and the store says which repository wrote it,
   * because `restore-keys` is a prefix match and can hand this run another checkout's file.
   *
   * Without these, nothing survives a run at all: the baseline is re-measured from cold, the
   * predictor, pool 2 and the line map start empty, no bug is carried across a push, and a second
   * push on the same pull request re-pays for every verdict it already has. Measured 2026-08-23: a
   * run sharing a triage cache carried 13 of 15 verdicts and cost $0.98 against $1.59.
   *
   * THE DIRECTORY COMES FROM THE DESCRIPTOR AND NOT FROM HERE, and nothing about the run is in it.
   * `actions/cache` versions an entry by its paths, so the `<run id>-<run attempt>` this boundary
   * used to put in the path made every save unreadable however well the keys matched.
   */
  const cacheDirectories = {};
  for (const cache of ACTION_CACHES) {
    cacheDirectories[cache.pathOutput] = privateDirectory(
      environment.RUNNER_TEMP,
      actionCacheSegments(cache),
      "fresh",
    );
  }

  /*
   * AND THE STATE THAT MUST NOT BE SHARED, under a root of its own so nothing can join it by
   * sitting beside something that belongs there.
   *
   * Deliberately fresh on every run, never restored from GitHub cache: cache contents are
   * repository-controlled and a self-authored receipt cannot make restored executable code trusted.
   * The RUN populates it within THIS run - a vitest repository with no `@vitest/coverage-v8` has the
   * pair staged into it by the measuring process itself, with npm and with lifecycle scripts
   * disabled, receipt-validated by the CLI before use. There used to be a separate "Prepare coverage
   * provider cache" step for that, running a customer command called `abloh prepare`; the captain
   * retired it on 2026-09-02 (census run 4's F5) because a workspace package the step's own walk
   * could not classify was staged for by nobody, and the run then refused for a provider its remedy
   * told the maintainer to install with the command that had just run. This variable is what still
   * confines the cache to `RUNNER_TEMP` and stops the CLI consulting a user-level cache outside the
   * Action boundary.
   */
  const runStateDirectory = privateDirectory(environment.RUNNER_TEMP, [
    RUN_STATE_ROOT,
    `${runId}-${runAttempt}`,
  ], "new");
  const coverageCacheDirectory = join(runStateDirectory, "coverage-providers-fresh");
...
export function buildRunArguments(environment = process.env) {
   ...
   */
  const triageCacheDirectory = realpathSync(required(environment.ABLOH_TRIAGE_CACHE_DIR, "the harmfulness review's cache directory"));
  if (!inside(runnerTemp, triageCacheDirectory, false)) fail("the harmfulness review's cache directory must stay inside RUNNER_TEMP");
  if (inside(repositoryRoot, triageCacheDirectory) || inside(triageCacheDirectory, repositoryRoot)) {
    fail("the harmfulness review's cache directory and the measured repository must not overlap");
  }
...
/**
 * THE SERVICE'S ADDRESS, NOT ONE OF ITS DOORS.
 *
 * WHY THIS EXISTS (the 2026-09-07 model-service contract audit, rank 6). This Action handed the CLI
 * the chat-completions path, and every endpoint resolver in this product treats a URL naming
 * `/chat/completions` as a customer's DELIBERATE routing decision - it says "send my traffic to this
 * one address", so no Responses sibling is derived from it. The service serves both surfaces, and
 * the one it could never be asked for on a hosted run is the one that survives the endpoint's idle
 * timer: `/chat/completions` emits nothing until the first output token, and a generation call that
 * reasons for two minutes is cut before it answers.
 *
 * IT NARROWS TO ROUTES THIS SERVICE DECLARES AND LEAVES EVERYTHING ELSE ALONE. A URL naming none of
 * them is somebody's own endpoint and is handed on exactly as it arrived - which is the same rule
 * `resolveResponsesUrl` follows on the other side of the wire.
 */
export function modelServiceEndpoint(url) {
  const trimmed = String(url ?? "").trim().replace(/\/+$/u, "");
  for (const path of MODEL_SERVICE_ROUTE_PATHS) {
    if (trimmed.endsWith(path)) return `${trimmed.slice(0, trimmed.length - path.length)}${MODEL_SERVICE_BASE_PATH}`;
  }
...
export async function runAbloh(environment = process.env, fetchImpl = fetch) {
  ...
  if (gatewayConfigured && condition !== null && modelTokenUsable) {
     ...
     */
    /* THE SERVICE AND NOT ONE OF ITS DOORS - see {@link modelServiceEndpoint} for what naming the
       chat path cost every hosted run. */
    childEnvironment.MODEL_ENDPOINT = modelServiceEndpoint(environment.MODEL_GATEWAY_URL);
    childEnvironment.MODEL_ENDPOINT_ALT = childEnvironment.MODEL_ENDPOINT;
     ...
     * triage lane and left marigold with no endpoint at all. v2 read `MODEL_*` and nothing wrote
    ...
    /* THE SERVICE AND NOT ONE OF ITS DOORS. The URL is still VALIDATED as the gateway target the
       operator configured - credential-free https, the audience beside it - and what the CLI is
       handed is the service that target belongs to. */
    childEnvironment.MODEL_ENDPOINT = modelServiceEndpoint(gateway.gatewayUrl);
    childEnvironment.MODEL_ENDPOINT_ALT = childEnvironment.MODEL_ENDPOINT;
    childEnvironment.MODEL_AUTH = "bearer";
...
/**
 * HOW MANY LINES OF ONE REFUSAL THIS LOG PRINTS.
 *
 * A refusal composes one line for the sentence, one per evidence slot, one for the disclosure, one
 * for the next action, one for the file and key, and one plus the value's own lines for the shape.
 * `RUN_REFUSAL_LIMITS.evidenceItems` is eight, so thirty-two is every line any refusal in the
 * registry can produce with room over - and what it bounds is not a refusal, it is a file on a disk
 * this process does not own. The byte ceiling above is the real defence; this stops a readable file
 * from turning one refusal into a thousand log lines.
 */
const MAX_RUN_REFUSAL_LINES = 32;
...
export function explainMissingArtifact(environment = process.env, write = process.stderr) {
  ...
  /* NO APPENDED FULL STOP (the displayer divergence audit of 2026-09-12, D5). Every registry summary
     ends its own sentence - `composedSummary` puts the stop on where a declaration left it off - so
     the one added here printed `...so there is no catch rate for it..` on every hosted refusal. */
  write.write(`abloh: no measurement was produced. ${record.summary}\n`);
  /* PRINTED WHERE THERE IS ONE, AND THE COMPOSER DECIDES (the captain, 2026-09-10). This line was
     once conditional on the record carrying a next action, then unconditional so a refusal that
     asks for nothing still said something, and is now conditional again on an answer the surface
     does not compute: `nextActionLine` returns null for the nothing-waiting state, so the job log
     drops the line exactly where the check run, the terminal, the comment and the run page do. */
  const nextLine = nextActionLine(record.nextAction, "");
  if (nextLine !== null) write.write(`abloh: ${nextLine}\n`);
  /*
   * AND THE REST OF THE REFUSAL, WHICH THIS SURFACE COULD NOT PRINT UNTIL 2026-09-12.
   *
   * The evidence with its labels and where each line came from, the disclosure of what stayed on the
   * machine, the file and key the remedy names and the value that key takes - the run composed all
   * of them and only the first two fields crossed. The sentence and the next action are printed
   * above in this surface's own shape, so the lines that repeat them are dropped rather than said
   * twice: `refusalLines` opens on the summary and carries the next action under it.
   */
  const said = new Set([record.summary, nextLine === null ? "" : record.nextAction]);
  for (const line of record.lines) {
    if (said.has(line) || line === `next: ${record.nextAction}`) continue;
    write.write(`abloh: ${line}\n`);
  }
...
export function readSweepLedger(outputDirectory) {
  for (const name of SWEEP_LEDGER_FILES) {
    ...
    const what = validating ? "an abloh init validation run" : "a diagnostic sweep";
    return {
      kind: "sweep-ledger",
      file: name,
      ...
      summary:
        `this run was ${what}, which measures nothing and writes no measurement artifact by design. ` +
        (walls.length === 0
          ? "It reached the end of every stage it ran and found nothing that stops a run."
          : `It found ${walls.length} stage(s) that stopped, and carried on past each one to find the rest.`),
      lines: [
        ...walls.map((wall) => {
          const stage = boundedLogString(wall.stage, 32) ?? "an unnamed stage";
          const summary = boundedLogString(wall.refusal?.summary, 500);
          const never = wall.neverStarted === true ? " This stage never started against that refusal." : "";
          return summary === null
            ? `refused at the ${stage} stage.`
            : `${endedWithOneStop(`refused at the ${stage} stage: ${summary}`)}${never}`;
        }),
        `the whole record is in ${name}, beside where the artifact would have gone.`,
      ],
    };
  }
  return null;
...
export function readRunRefusal(path) {
  ...
  const summary = boundedLogString(parsed.summary, RUN_REFUSAL_LIMITS.summary);
  if (summary === null) return null;
  return {
    summary,
    nextAction: boundedLogString(parsed.nextAction, RUN_REFUSAL_LIMITS.remedyText),
    code: boundedLogString(parsed.code, RUN_REFUSAL_LIMITS.code) ?? "unnamed",
    owner: boundedLogString(parsed.owner, RUN_REFUSAL_LIMITS.code) ?? "unnamed",
    stage: boundedLogString(parsed.stage, RUN_REFUSAL_LIMITS.code) ?? "unnamed",
    reportId: boundedLogString(parsed.reportId, RUN_REFUSAL_LIMITS.reportId),
    /*
     * THE WHOLE REFUSAL, AS THE RUN'S OWN LINES (the displayer divergence audit of 2026-09-12, A4).
     *
     * The six fields above are what used to cross, so this surface had the sentence and the next
     * action and could not print the evidence, the file the remedy names, the key inside it, the
     * value that key takes, or the disclosure of what stayed on the machine - all of which the run
     * had in hand. They arrive composed, by `refusalLines`, which is the terminal's own composer:
     * this process can import nothing, so a second composition here would be a second set of words.
     *
     * ABSENT ON A RUN OLDER THAN THE FIELD, and the log then reads exactly as it did.
     */
    lines: Array.isArray(parsed.lines)
      ? parsed.lines
          .slice(0, MAX_RUN_REFUSAL_LINES)
          .map((line) => boundedLogString(line, RUN_REFUSAL_LIMITS.summary))
...
/**
 * A string on its way to a job log: control characters flattened, length held to the WIRE'S bound.
 *
 * THE BOUNDS ARE NOT THIS FILE'S ANY MORE (the displayer divergence audit of 2026-09-12, C3). They
 * were 500 for a sentence, 500 for a next action, 64 for a code and 32 for an owner, typed here,
 * with no relation to any bound the producer had been held to - so this surface cut registry copy
 * that had already crossed the run's own projection intact. `RUN_REFUSAL_LIMITS` is the one place
 * those numbers live, and it reaches this process through `wire-contract.generated.mjs` for exactly
 * the reason two copies of one number is one number that can be two numbers.
 *
 * WHAT IT STILL DOES IS FLATTEN CONTROL CHARACTERS, which is not shortening: a log's own framing is
 * the thing being defended, and every character this removes renders as nothing anyway.
 */
...
export async function reportSetupTrial(
  ...
  if (url === "") {
    narrate("abloh: the setup report stays in this run - no control plane is configured for it");
    return 0;
  }
  ...
  for (let attempt = 1; ; attempt += 1) {
    ...
    if (response !== null && !RETRYABLE_UPLOAD_STATUS(response.status)) {
      /* THE SENTENCE IS THE REGISTRY'S and the status is a figure this door adds after it, so the
         line is composed here and written rather than written here. */
      const publishFailed = identityConditionLine("identity-issuance-or-publish-failed", {
        stage: "publish",
        attempt: attempts,
        attempts,
      }).trimEnd();
      narrate(`${publishFailed} (HTTP ${response.status}).`);
      return 0;
    }
...
export async function stageForAttestation(environment = process.env) {
  ...
  if (staged === 0) {
    process.stdout.write(
      "abloh: this run produced nothing to file, so the publish job has nothing to do\n",
    );
  }
  return 0;
}
...
export function stageRunEvidence(environment = process.env) {
  ...
  const { manifest, problem } = readEvidenceManifest(resultsPath);
  ...
  for (const name of AMBIENT_ENDPOINT_OVERRIDES) {
    if (typeof environment[name] === "string" && environment[name] !== "") {
      fail(
        ...
        "publish step where a reviewer can see it beside the permission it affects",
      );
    }
  }
}
...
export async function attest(
  ...
  const directory = optional(environment.ABLOH_ATTESTATION_DIR, "the directory the publish job reads");
  ...
  if (!lstatSync(canonical).isDirectory()) fail("the directory the publish job reads is not a directory");
...
export async function buildEvidenceEnvelope(environment = process.env) {
   ...
   */
  const { manifest, problem: manifestProblem } = readEvidenceManifest(resultsPath);
  reportMissingManifest(manifestProblem);
  const { evidence: recordEvidence } = readRecordEvidence(resultsPath);
  /*
   * THE FOUR DOCUMENTS, READ OUT OF THE RECORD THEY ARE IN (the captain's ruling of 2026-09-14).
   *
   * NOTHING ABOUT THE ENVELOPE MOVED. The keys below are the keys this step has always posted -
   * what changed is that the bytes come from four MEMBERS of the artifact this process has already
   * parsed rather than from four files beside it, which is what makes the record a maintainer opens
   * exactly what the upload carries.
   *
   * COVERAGE IS ALREADY THE REPORT. It used to arrive wrapped in an envelope carrying the per-suite
   * documents a merging run derived it from, which this step unwrapped away before every upload -
   * they are LOCAL evidence and always were, and a monorepo's are N times the report, so forwarding
   * them would push a real repository through the acceptor's own bound and lose the coverage view
   * entirely. They are in `abloh-run.local.json` now, so there is nothing left to unwrap.
   *
   * THE SIZE BOUND DID NOT GO WITH THE FILES, and it is applied by the one function both roads ask
   * (`boundRunEvidence`): a member over `SIDECAR_MAX_BYTES` would take the WHOLE envelope with it at
   * the door, and a document held back is NAMED rather than dropped in silence.
   */
  const bounded = boundRunEvidence(recordEvidence, SIDECAR_MAX_BYTES);
  const withheldSidecars = bounded.withheld.map((entry) => ({
    basename: entry.name,
    bytes: entry.bytes,
    cause: "oversize",
  }));
  const sidecars = bounded.sidecars;
  ...
  reportPackageSidecars(auditPackageSidecars(evidence, recordEvidence));
  /*
...
export async function postEvidenceEnvelope(
  ...
  if (response.status === 402) {
    ...
    narrate(
    `abloh: evidence not uploaded - ${message}`,
  );
    ...
    if (nextAction !== null) narrate(
    `abloh: next: ${nextAction}`,
  );
    return 0;
  }
...
const WITHHELD_SIDECAR_CAUSES = {
  oversize:
    `a sidecar is limited to 16 MiB. This is usually one pull request touching an unusually large ` +
 ...
 * v2 sidecar PER PACKAGE, under `abloh-proposals-packages/<slug>/`, and each package's block in the
 ...
 * been left behind.
 *
 * THE ARTIFACT IS THE MANIFEST'S AUTHORITY. The package list comes from `evidence.proposals.packages`
 ...
 */
export function auditPackageSidecars(evidence, recordEvidence = null) {
  /* Either name the artifact wrote the block under - see `build-handoff.mjs`'s own reading of it
     for why a published Action meets CLIs on both sides of the 2026-08-31 rename. */
  const packages = (evidence?.proposals ?? evidence?.engineV2)?.packages;
  if (!Array.isArray(packages) || packages.length === 0) return [];
  /* WHERE EACH PACKAGE'S EVIDENCE IS, ASKED OF THE RECORD (the captain's ruling of 2026-09-14). It
     was a directory under a slug of the package name plus an index file naming the slugs, which is
     what this Action used to read; an entry names its own package directory now, so there is no
     slug rule to re-derive and nothing to resolve onto a path. */
  const listed = new Map();
  for (const row of recordEvidence?.packages ?? []) {
    if (typeof row?.directory === "string" && typeof row?.proposalsProofs === "string") {
      listed.set(row.directory, row.proposalsProofs);
    }
  }
  const records = [];
  for (const entry of packages) {
    ...
    /* THE DIGEST IS ON THE PACKAGE'S OWN BLOCK, WHICH IS WHERE THE PRODUCER PUT IT (finding E,
       ...
       in `packages/marigold/src/composition.ts` is the authority and reads
       ...
       false clauses over evidence that was present, signed and correct.
    ...
    const bytes = listed.get(directory);
    /* UNLISTED IS A RECORD THAT CARRIES NO ENTRY FOR THIS PACKAGE, which is what an older CLI's
       artifact looks like: it wrote the evidence to a directory this Action no longer reads. */
    if (bytes === undefined) {
      records.push({ directory, path: "", state: "unlisted" });
      continue;
    }
    const path = `${RECORD_EVIDENCE_PATH.proposalsProofs} (${directory})`;
    if (expected === null) {
      records.push({ directory, path, state: "unreadable" });
      continue;
    }
 ...
 */
function readEvidenceManifest(resultsPath) {
  const { evidence, problem } = readRecordEvidence(resultsPath);
  if (evidence === null) return { manifest: null, problem };
  if (evidence.index === null) return { manifest: null, problem: "unreadable" };
  return {
    manifest: { schema: EVIDENCE_MANIFEST_SCHEMA, outputs: evidence.index.map((row) => ({ ...row })) },
    problem: null,
  };
}

/**
 * THE RECORD'S OWN EVIDENCE BLOCK, or null with the state that says why.
 *
 * SINCE THE 2026-09-14 FOLD the four documents this step forwards and the index over the directory
 * are MEMBERS of the artifact rather than files beside it, so the read is one `JSON.parse` of a
 * file this process has already proved is complete. `packages/core/src/run-evidence.ts` carries the
 * ruling; nothing about the envelope moved, only where these bytes are read from.
 *
 * `absent` IS A CLI OLDER THAN THIS ACTION, which is the ordinary cross-version pairing - a
 * customer's workflow pins this Action by SHA and the CLI it installs is resolved at run time - and
 * the one thing it must not be is silent.
 */
function readRecordEvidence(resultsPath) {
  let record;
  try {
    record = JSON.parse(readFileSync(resultsPath, "utf8"));
  } catch {
    return { evidence: null, problem: "unreadable" };
  }
  if (typeof record !== "object" || record === null || Array.isArray(record)) {
    return { evidence: null, problem: "unreadable" };
  }
  const evidence = parseRunEvidence(record.evidence);
  return { evidence, problem: evidence === null ? "absent" : null };
}
 ...
 *
 * ONE OF THE THREE CAUSES IS VERSION DRIFT, AND IT TAKES THE SAME NEXT ACTION THE OTHER TWO DO.
 * `unknown-output` is a name this build has never heard of, which is the identical fact
 * `unrecognisedCoverageReasonNotice` and `uploadTruncatedNotice` report one file over - a workflow
 * pins the Action by SHA while the CLI it installs is resolved at run time. Those two close on
 * `ACTION_PIN_REMEDY` and this one closed on nothing at all, so a maintainer was told their document
 * was dropped and left with no move to make. Joined here rather than in core because
 * `evidence-registry.ts` is rendered by `pnpm gen:action`, which refuses a source module with an
 * import; `handoff-envelope.mjs` joins the same two owned sentences the same way at both of its
 * drift sites. The OTHER two causes take no remedy: neither is about a version.
 */
function reportWithheldEvidence(entries) {
  for (const entry of entries) {
    const line = evidenceWithheldLine(entry);
    const said = entry.reason === "unknown-output" ? `${line} ${ACTION_PIN_REMEDY}` : line;
    process.stdout.write(`${said}\n`);
  }
}

/**
 * Name every document the record carried that is not in the envelope.
 *
 * THE MEMBER PATH, never a runner path: an absolute path in a public build log is the same leak the
 * refused-upload branch above avoids, and the member is inside the record beside the artifact where
 * the customer already knows to look. The step still exits 0 - the measurement ran and said whatever
 * it said, and only a document that describes it is missing.
 */
function reportWithheldSidecars(withheld) {
  for (const entry of withheld) {
    const mib = (entry.bytes / (1024 * 1024)).toFixed(1);
    narrate(
      `abloh: ${entry.basename} (${mib} MiB) was not uploaded - ` +
        `${WITHHELD_SIDECAR_CAUSES[entry.cause]}. The run's score is unaffected. The evidence this ` +
        `document carried is not on the pull request.`,
    );
  }
}
...
async function main(environment = process.env) {
  ...
  if (command === "preflight") {
    ...
    appendOutput(environment.GITHUB_OUTPUT, {
      ...
      /* ONE OUTPUT PER DESCRIBED CACHE, named by the descriptor that both its steps read the path
         from. A cache declared in `packages/core/src/action-caches.ts` gets its restore step, its
         save step and the output they resolve their path through from that one declaration. */
      ...result.cacheDirectories,
...
if (import.meta.url === pathToFileURL(process.argv[1] ?? "").href) {
  try {
    ...
    /* WHATEVER THREW SAID IT: an `ActionRefusalError` carries the registry's own words and an
       invariant carries a statement about abloh's code. This door renders, and composes nothing. */
