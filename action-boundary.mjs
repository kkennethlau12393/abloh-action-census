...
import { PR_COMMENT_REMEDY, prCommentUnavailableNotice } from "./action-notices.generated.mjs";
...
import {
  coverageReportBytes,
  PROPOSALS_PACKAGE_SIDECAR_INDEX,
  evidenceBasename,
...
export function preflight(environment = process.env) {
   ...
   * already has.
   */
  const marigoldDirectory = join(stateDirectory, "marigold");
  /*
  ...
  mkdirSync(marigoldDirectory, { mode: 0o700 });
...
export async function buildEvidenceEnvelope(environment = process.env) {
  ...
  const sidecars = {
    ...
    /* The v2 engine's proofs. What bounds it is the acceptor, which verifies the bytes against
       `proposals.proofsDigest` and keeps only candidates that PROVED. */
    proposalsProofs: readOptionalFile(sidecarPath("proposals-proofs"), withheldSidecars),
    /* Pool 2's evidence: every agent-written bug this run handled, with its witness test. A second
       ...
       `proposals.disclosure.agentBugs.evidenceDigest`. Bounded the same way: the acceptor verifies
       the bytes against that digest and keeps only bugs the artifact SIGNED as survivors. */
    proposalsPool2: readOptionalFile(sidecarPath("proposals-pool2"), withheldSidecars),
  };
...
export function auditPackageSidecars(evidence, outputDirectory, index = null, readFile = readFileSync) {
  ...
  try {
    /* THE DIRECTORY'S NAME COMES FROM THE RUN, like every other name this step resolves. It was
       `"abloh-proposals-packages"` written out here until 2026-09-02 - this Action's opinion about
    ...
    const sidecars = JSON.parse(
      readFile(
        join(
          outputDirectory,
          storedEvidenceBasename(index?.outputs ?? null, "proposals-packages"),
          PROPOSALS_PACKAGE_SIDECAR_INDEX,
        ),
        "utf8",
      ),
  ...
  for (const entry of withheld) {
    ...
    narrate(
      `abloh: ${basename(entry.path)} (${mib} MiB) was not uploaded - ` +
        ...
        `file carried is not on the pull request.`,
    );
  }
}
...
async function main(environment = process.env) {
  ...
  if (command === "preflight") {
    ...
    appendOutput(environment.GITHUB_OUTPUT, {
      ...
      "v2-store-dir": result.marigoldDirectory,
