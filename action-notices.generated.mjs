...
/**
 * A FINDING WHOSE PATH THE WIRE CANNOT CARRY, named where the run can still say so.
 *
 * WHY THE RUN STOPS RATHER THAN DROPPING THE ROW (boundary contract divergence audit, D14). The
 * ingest door bounds a finding's `file` and refuses the WHOLE envelope over it - and it cannot do
 * anything else, because the same door requires exactly one finding per survivor: dropping the row
 * is how a caller would hide a real gap behind a clean-looking certificate, and that invariant was
 * written after a run with two survivors and an empty findings list was accepted.
 *
 * So the row cannot go and the envelope cannot go without it. What was wrong is that nobody SAID
 * so: the projection formed the path, the door refused the envelope, and the customer got a bare
 * HTTP 400 after a complete measurement. This is the same posture the finding cap already takes one
 * function above - refuse here, by name, with the whole measurement still in the job's artifact.
 */
export function findingPathOverWireNotice(facts) {
    return (`a finding names a file path of ${facts.length} characters and one upload carries at most ` +
        `${facts.limit}: ${facts.file.slice(0, 120)}. Nothing was uploaded, because the control plane ` +
        "requires one finding per survivor and dropping the row would describe a run with fewer gaps " +
        "than this one found. The whole measurement is in this job's own artifact");
}
