/**
 * THE ONE COMPOSER OF THE `abloh-ci-handoff/v2` ENVELOPE, FOR BOTH ROADS.
 *
 * WHY IT IS ITS OWN FILE. It used to sit in `build-handoff.mjs` beside that file's `main()`, which
 * reads `ABLOH_EVIDENCE_PATH` and runs itself whenever it is the entry point. A signed-in local run
 * files the same envelope a pull-request run files, and the CLI is a bundle: importing a module
 * that self-runs would have made `abloh run` throw at start-up. So the composer moved here, where
 * nothing runs on import, and `build-handoff.mjs` is the runner-side entry point that imports it.
 *
 * ONE COMPOSER, TWO ROADS, DECIDED BY `ctx.road`. The `evidence` block, the sidecars and the index
 * are composed identically whichever road produced the run - that is the whole point, and it is
 * what `apps/cli/src/local-run-evidence.test.ts` holds byte for byte. What differs is `provenance`,
 * and only where the fact itself differs: a laptop has no workflow, no job and no pull request, so
 * the four GitHub keys are ABSENT rather than blank, and the road is named on the envelope.
 *
 * CONTRACT: the hosted output must remain byte-identical to the jq filter this replaced.
 * `build-handoff.differential.test.mjs` enforces that against every corpus artifact by running both
 * and comparing the serialized result. Two consequences for anyone editing this file:
 *
 ...
 *      deliberately not `??`.
 *
 * The endpoint it feeds derives repository identity from GitHub's signed OIDC token on the hosted
 * road, so no field here may carry a runner-local path.
 */
...
import {
  ACTION_PIN_REMEDY,
  COVERAGE_REASON_SUBSTITUTE,
  NARROW_THE_CHECK_REMEDY,
  findingPathOverWireNotice,
  findingsOverUploadCapNotice,
  unrecognisedCoverageReasonNotice,
  uploadTruncatedNotice,
} from "./action-notices.generated.mjs";

/** This Action's account of what it carried. The rule is in `packages/core/src/narration.ts`. */
...
import {
  ...
  FINDING_FILE_PATH_MAX,
  MAX_UPLOADED_FINDINGS,
  RUN_FINDING_ABSENCE,
  RUN_FINDING_FIELDS,
  RUN_FINDING_NESTED,
  EVIDENCE_SOURCES,
  ...
  REFUSAL_STAGES,
  RUN_INTERRUPTION_FACT_KEY_RE,
  RUN_INTERRUPTION_LIMITS,
  RUN_REFUSAL_CODE_RE,
  RUN_REFUSAL_LIMITS,
  RUN_STAGES,
  RUN_STAGE_UNITS,
...
/* The finding projection's own rule, rendered from `packages/core/src/wire/run-finding-projection.ts`.
   See audit D01: the rule that decides how an absence is spelled had two authors and they meant
   different things by an absent `coveredBy`. */
import { projectByAbsence } from "./wire-projection.generated.mjs";
/*
...
export function normalizeL0Reason(reason) {
  if (typeof reason !== "string") {
    throw new InvariantError("diff coverage `cannot-attest` reason is required");
  }
  ...
  narrate(`abloh: ${unrecognisedCoverageReasonNotice()}. ${ACTION_PIN_REMEDY}`);
  return COVERAGE_REASON_SUBSTITUTE;
}
...
function bounded(values, limit, what) {
  ...
  narrate(`abloh: ${uploadTruncatedNotice({ reported: values.length, limit, what })}. ${ACTION_PIN_REMEDY}`);
...
/*
 * WHAT USED TO SIT HERE, AND WHY IT IS GONE (boundary contract divergence audit, D01 / F1).
 *
 * `FINDING_KEYS` and `TRIAGE_KEYS` were hand-kept allowlists beside two more in the ingest door -
 * its egress sanitizer's names and its value validator's rules - and a producer that formed the fact
 * in a third place. Four accounts of one shape, and they disagreed twice over:
 *
 *   `coveredBy` - the producer OMITS it when it measured no covering population, `pick` wrote that
 *   absence as `null`, and the door refuses a null. Two `expressjs/express` records were refused
 *   after a complete measurement.
 *
 *   `about`, `severity` and `severityBasis` - the door admits, stores and SERVES all three, and this
 *   list carried none of them, under a comment claiming the two lists held the same names. Three
 *   hosted readers already written, already tested on their own side, and unable to fire.
 *
 * `RUN_FINDING_FIELDS`, `RUN_FINDING_ABSENCE` and `RUN_FINDING_NESTED` are the contract's own tables,
 * generated by `pnpm gen:wire`; `projectByAbsence` is the contract's own rule, generated by
 * `pnpm gen:action`. `TIER2_FINDING_KEYS` was a fifth list here and is deleted with the ladder: the
 * contract declares no source-bearing field, so no projection can carry one.
 */
 ...
 */
/*
 * THE ROW'S KEYS AND ITS EXCLUSION BOUND COME FROM THE CONTRACT.
 *
 * They were authored here and again in the ingest door, and held together by a test that compares
 * the two lists - a pin that can only fail in a checkout holding both sides, never on a customer's
 * Action pinned by SHA meeting a control plane released afterwards. `excluded` is dropped from the
 * projected list because it is built by `excludedBlock` rather than copied; the contract still
 * declares it, which is what makes the door's set and this one one fact.
 */
const PACKAGE_ROW_KEYS = PACKAGE_ROW_FIELDS.filter((key) => key !== "excluded");
const MAX_EXCLUSION_STR_LEN = PACKAGE_EXCLUSION_TEXT_MAX;
...
/**
 * The most findings one upload may carry, RE-EXPORTED from the contract that declares it.
 *
 ...
 * job's own artifact.
 *
 * THE NUMBER MOVED TO `wire/run-finding.ts` and is generated back in: it is a fact about the
 * BOUNDARY, and a bound one end refuses over is one both ends have to read. The name stays exported
 * here because this module's own suite imports it.
 */
export { MAX_UPLOADED_FINDINGS };

/**
 * Findings are projected by the wire contract, on every run.
 *
 * `replacement`, `originalText` and the exact columns are customer SOURCE and are not fields of the
 * contract, so they are dropped here by not existing there. There is no level that widens this: the
 * tier ladder that once did is deleted, and the contract is the whole of it.
 */
function findingEntries(findings) {
  const all = alt(findings, []);
  if (all.length > MAX_UPLOADED_FINDINGS) {
    throw new ActionRefusalError(
      `${findingsOverUploadCapNotice({ reported: all.length, limit: MAX_UPLOADED_FINDINGS })}. ` +
        NARROW_THE_CHECK_REMEDY,
    );
  }
  /*
   * A PATH THE WIRE CANNOT CARRY STOPS THE RUN HERE, BY NAME (audit D14).
   *
   * The door bounds this field and refuses the whole envelope over it, and it cannot drop the row
   * instead: it requires exactly one finding per survivor, because a short findings list is how a
   * real gap would hide behind a clean certificate. So the choice is a bare HTTP 400 after a
   * complete measurement, or this - the same posture the finding cap above already takes.
   */
  for (const finding of all) {
    const file = finding?.file;
    if (typeof file === "string" && file.length > FINDING_FILE_PATH_MAX) {
      throw new ActionRefusalError(
        `${findingPathOverWireNotice({ file, length: file.length, limit: FINDING_FILE_PATH_MAX })}. ` +
          NARROW_THE_CHECK_REMEDY,
      );
    }
  }
  return all.map((finding) =>
    projectByAbsence(finding, RUN_FINDING_FIELDS, RUN_FINDING_ABSENCE, RUN_FINDING_NESTED),
  );
}
 ...
 * @param {{coverage?: string, mutationRedacted?: string, proposalsProofs?: string, proposalsPool2?: string}} [sidecars]
 ...
 *
 *   `proposalsProofs` is bounded by its acceptor, which verifies the bytes against
 *   `proposals.proofsDigest` and stores only candidates that PROVED.
 *
 *   `proposalsPool2` travels on the same terms, bound to its own commitment
 *   (`proposals.disclosure.agentBugs.evidenceDigest`) and projected to the bugs the artifact signed
 *   as survivors.
 *
...
/**
 * WHAT STOPPED THE RUN, BOUNDED, or null.
 *
 * THE RECORD CARRIES NO PROSE BY CONSTRUCTION - a mechanism, a stage, three integers and the
 * mechanism's own coordinates - so unlike an engine failure there is nothing here to redact. What
 * this does is BOUND it and hold every closed name to the vocabulary this build was published with,
 * which is `engine-failure-id.ts`'s two-wall rule: the runner checks the grammar and the control
 * plane checks membership, because the two are separately deployed and the pinned one is older.
 *
 * A RECORD THIS BUILD CANNOT READ IS DROPPED, NEVER FORWARDED IN PART. Half an interruption is a
 * claim nobody made, and the measurement it describes is complete either way - so dropping it costs
 * a sentence, where forwarding a shape the door refuses would cost the whole envelope.
 */
function interruptionBlock(interruption) {
  if (interruption === null || interruption === undefined || typeof interruption !== "object") return null;
  if (!INTERRUPTION_MECHANISMS.includes(interruption.mechanism)) return null;
  if (!RUN_STAGES.includes(interruption.stage)) return null;

  let progress = null;
  const raw = interruption.progress;
  if (raw !== null && raw !== undefined) {
    const count = (n) =>
      typeof n === "number" && Number.isInteger(n) && n >= 0 && n <= RUN_INTERRUPTION_LIMITS.maxProgress ? n : null;
    const done = count(raw.done);
    const total = count(raw.total);
    if (done === null || total === null || !RUN_STAGE_UNITS.includes(raw.unit)) return null;
    progress = { done, total, unit: raw.unit };
  }

  const facts = {};
  for (const [key, value] of Object.entries(interruption.facts ?? {})) {
    if (Object.keys(facts).length >= RUN_INTERRUPTION_LIMITS.maxFacts) break;
    if (!RUN_INTERRUPTION_FACT_KEY_RE.test(key)) continue;
    if (typeof value === "number") {
      if (Number.isFinite(value)) facts[key] = value;
      continue;
    }
    /* A job reference is a path a customer wrote, so it is bounded and stripped of anything that
       would make a stored document unrenderable - the same wall every other text field here keeps. */
    if (typeof value === "string") facts[key] = printableLine(value, RUN_INTERRUPTION_LIMITS.maxFactValue);
  }
  return { mechanism: interruption.mechanism, stage: interruption.stage, progress, facts };
}
...
export function buildStructuralHandoff(evidence, ctx, sidecars, evidenceIndex) {
  return {
    ...
    /*
     * WHO PRODUCED THIS RUN AND WHERE, and the ONE block that differs by road.
     *
     * THE HOSTED SHAPE IS UNMOVED, key for key and in this order, because the differential test
     * compares it byte for byte against the jq filter it replaced.
     *
     * THE LOCAL SHAPE NAMES ITS ROAD AND OMITS WHAT A LAPTOP HAS NOT GOT. A workflow reference, a
     * workflow commit, a job id and an attempt are GitHub's facts about a GitHub job, and a run on
     * somebody's own machine has none of them - so they are ABSENT rather than blank. An empty
     * string there would be this producer claiming a workflow it could not name, and the door would
     * refuse it for the shape rather than admit it for the meaning. `road` is what the receiving
     * door reads to know which of the two shapes it is holding.
     */
    provenance:
      ctx.road === "local"
        ? {
            ...
            headSha: ctx.headSha,
            /* No pull request exists on this road. It is stated rather than omitted because every
               reader of a provenance block asks this question first. */
            pullRequest: null,
            /* The branch this run measured, in git's own words, or null on a detached checkout. */
            ref: ctx.ref === "" || ctx.ref === undefined ? null : ctx.ref,
          }
        : {
            repository: ctx.repository,
            triggerSha: ctx.triggerSha,
    ...
    evidence: {
       ...
       *
       * The control plane refuses a `proposals` block that carries a `tier` field at all, and that
       ...
       * ingest by `sanitizeProposals`, which refuses a forbidden or unknown one outright, so a
       ...
       */
      /* EITHER NAME THE ARTIFACT WROTE IT UNDER. The engine is MARIGOLD since the captain's naming
         ruling of 2026-08-31 and the block took the plain word instead of either engine name, but
         this Action is published to its own repository at a pinned SHA and the CLI it installs is
         resolved at run time - so a newer Action reads artifacts from CLIs on both sides of that
         rename, and reading only the new key would forward `null` for a run that measured. What
         goes UP is the new name alone; the control plane takes both at its own door for the
         opposite pairing. */
      proposals: (() => {
        const block = evidence.proposals ?? evidence.engineV2;
        return block === null || block === undefined
          ? null
          : {
              ...block,
              /* Same scrub as `fixLoop.reason`, for the same reason: one proposals failure path
              ...
              ...(typeof block.reason === "string" ? { reason: printableLine(block.reason) } : {}),
            };
      })(),
      ...
      /*
       * MUTABLE SITES ON CHANGED LINES NO TEST EXECUTES, and OMITTED rather than null when coverage
       * was never read.
       *
       * ABSENT AND ZERO ARE DIFFERENT ANSWERS, which is the whole reason this key is emitted the way
       * `rewrittenFiles` is rather than the way `mutationWallMs` is. A `0` is a MEASUREMENT -
       * coverage was read, the uncovered lines are known, and the catalogue found nothing to plant
       * on them. An absence means this run never established which changed lines a test runs, and a
       * null forwarded on every envelope would say the first thing about every run that did the
       * second. `unguardedSitesFrom` in core is the one predicate that keeps them apart, and every
       * hosted reader already asks it.
       *
       * IT HAD FOUR READERS AND NO WRITER UNTIL NOW (artifact divergence audit, class 1). The ingest
       * door stores it, the check run and the sticky comment render an Unguarded sites row, Slack
       * states the count, and the run page has its own - all four written, all four tested on their
       * own side, and not one of them able to fire on any run ever made, because this projection did
       * not carry the key. Kenneth's ruling of 2026-09-01 is that the count is reported on EVERY
       * surface; on the hosted road that had never been true.
       */
      ...(typeof evidence.unguardedSites === "number" && Number.isInteger(evidence.unguardedSites) && evidence.unguardedSites >= 0
        ? { unguardedSites: evidence.unguardedSites }
        : {}),
      /*
       * WHAT STOPPED THIS RUN, and OMITTED rather than null when nothing did.
       *
       * NEARLY EVERY RUN IS UNINTERRUPTED, so an always-present null would put a key on every
       * envelope in circulation to say so - the same reasoning `rewrittenFiles` above is emitted on.
       * The receiver takes it either way: it is a widened-only key, permitted and never required, so
       * absent is the same answer as a producer released before it existed, and that answer is
       * "nothing stopped this run".
       */
      ...(interruptionBlock(evidence.interruption) === null
        ? {}
        : { interruption: interruptionBlock(evidence.interruption) }),
      /*
       * THE RUN'S OWN DURATION, one monotonic span, measured by the run.
       *
       * THE DASHBOARD USED TO SUM THE STAGES INSTEAD, which is precisely the derivation the producer
       * abandoned and wrote down why: summing only the stages that completed reported 0ms for a
       * red-baseline abort and silently omitted a provider failure - exactly the runs whose real cost
       * an operator most needs to see. The page did it because the key was not on the wire, so the
       * only number it could get was one it made up out of the parts.
       */
      wallMs: field(evidence.wallMs),
...
function sidecarPayload(sidecars) {
   ...
   * verified against `proposals.proofsDigest`, and only PROVEN candidates survive the join to
   * `exitProofs` there.
   *
   ...
   */
  if (typeof sidecars.proposalsProofs === "string" && sidecars.proposalsProofs.length > 0) {
    payload.proposalsProofs = sidecars.proposalsProofs;
  }
  /*
   ...
   * `proposals.disclosure.agentBugs.evidenceDigest` and then keeps only the bugs the ARTIFACT signed
   ...
   */
  if (typeof sidecars.proposalsPool2 === "string" && sidecars.proposalsPool2.length > 0) {
    payload.proposalsPool2 = sidecars.proposalsPool2;
  }
