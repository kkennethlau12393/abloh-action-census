 ...
 */
/**
 * THE IDENTIFIER THIS CODE QUOTES WHEN THE BOUNDARY HAS NONE OF ITS OWN.
 *
 * A hand copy of the declared identifier in `packages/core/src/refusal.ts`, held equal by
 * `scripts/upload-refusal-parity.test.ts`. It exists because an abloh-owned defect a maintainer
 * cannot report is a defect nobody learns about (the ownership audit, 2026-09-07, finding 3), and
 * this boundary is a standalone `.mjs` that cannot import the registry.
 */
export const UPLOAD_NO_ARTIFACT_REPORT_ID = "upload-no-artifact";
...
export function uploadNoArtifactLines({ expectedPath, reportId }) {
  ...
  lines.push(
    ablohFailureClosingLine(
      typeof reportId === "string" && reportId !== "" ? reportId : UPLOAD_NO_ARTIFACT_REPORT_ID,
    ),
  );
